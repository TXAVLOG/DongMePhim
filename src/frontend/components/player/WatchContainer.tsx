...
export const WatchContainer: React.FC<WatchContainerProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchUserAndAds = async (username: string) => {
      try {
        ...
        // Pre-roll ads logic (shown to free users or users whose package has ads_only_in_player)
        const ads = settings.ads || {};
        const bypass = perms?.bypass_ads || false;
        const allowPreRoll = !bypass;

        if (ads.pre_roll_enable && ads.pre_roll_url && allowPreRoll) {
  ...
  }, [movie.slug, currentEpisode?.slug, adPending, showAd]);
                  ...
                  <div className="absolute bottom-0 right-3 flex items-center gap-3 pb-2" style={{ zIndex: 210 }}>
                    {canSkipAd ? (
                      <button 
                        ...
                        className="px-4 py-2 bg-[#d2bbff] text-slate-950 font-black rounded-lg text-[11px] hover:brightness-110 active:scale-95 transition-all shadow-[0_0_20px_rgba(210,187,255,0.4)] flex items-center gap-1.5 border-none cursor-pointer"
                      >
                        <span>Bỏ qua quảng cáo</span>
                        <span className="material-symbols-outlined text-sm font-bold">skip_next</span>
                      </button>
                    ) : adCountdown <= 5 ? (
                      <div className="px-4 py-2 bg-black/80 backdrop-blur-md border border-white/10 rounded-lg text-[10px] text-white font-bold tracking-wider uppercase shadow-lg">
                        Bỏ qua ({adCountdown})
                      </div>
                    ) : null}
                  </div>
