  ...
  try {
    ...
    if (job === 'all') {
      ...
      for (const j of validJobs) {
        try {
          const params = j === 'sync-kkphim' ? { limit: limit || 30, resume: resume !== undefined ? resume : true } : undefined;
      ...
      const params = job === 'sync-kkphim' ? { limit: limit || 30, resume: resume !== undefined ? resume : true } : undefined;
